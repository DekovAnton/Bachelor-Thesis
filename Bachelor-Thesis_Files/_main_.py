...
class Worker(QThread):
    ...
    def simulate(self):
        try:
            ...
            toolvdc.write("DISP:MENU:NAME 0")
            ...
            for i in self.curvalues:
                ...
                self.progress = self.progress + point
...
class MainWindow(QMainWindow):
    ...
    def runqdialog(self):
        ...
        dialog=QDialog(self)
        uic.loadUi("GUI_PopUp_COMSettings.ui",dialog)

        # Find Comports
        try:
            self.SerialComportsidn=[]
            self.SerialComports=[]
            self.rm = pyvisa.ResourceManager()
            self.SerialComportsraw = list(self.rm.list_resources())
            for index in self.SerialComportsraw:
                try:
                    com=self.rm.open_resource(index).query("*IDN?").strip()
                    self.SerialComportsidn.append(com)
                    self.SerialComports.append(index)
                except Exception as e:
                    print(e)
    ...
    def process_Data(self,measure_dict):
        try:
            ...
            for key,value in measure_dict.items():
                try:
                    measure_dict[key]=[float(x) if "TimeStamp" not in key else x for x in value]
                except Exception as e:
                    print(e)
            ...
            self.df_toSave=pd.concat([self.df_toSave,df])
            self.df_toSave.reset_index(drop=True,inplace=True)
    ...
    def Save(self):
        try:
            ...
            if file_name:
                if not file_name.endswith(".xslx"):
                    file_name += '.xlsx'
                self.df_toSave.to_excel(file_name,index=False)
        except Exception as e:
            print(e)
